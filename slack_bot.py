...
def process_package_request(message_text, channel_id, message_ts):
    ...
    try:
        ...
        evaluation_results, evaluation_markdown = asyncio.run(evaluate_package_team(message_text))
        ...
        package_version = package_info.get("Requested_Package_Version", 
                                       package_info.get("Latest_Package_Version", "latest"))
        """ 

        
        # Check if the report contains an error message
        if evaluation_markdown is None:
